...
def trainer_SubEnv(json_path):
    # Get the parameters from the json file
    parameters_dict = model_json_to_parameters(json_path)
...
def trainer_PVAE(json_path):
    # hidden_size, deepness, frame_size, N_filter_bank, sr, model, latent_dim, atoms_size, atoms_number
    # Get the parameters from the json file
    parameters_dict = model_json_to_parameters(json_path)
    ...
    VAE_model_directory   = parameters_dict['VAE_model_directory']    
    VAE_latent_dim   = parameters_dict['VAE_latent_dim']
    VAE_atoms_size   = parameters_dict['VAE_atoms_size']
    atoms_number     = parameters_dict['atoms_number']
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    #Load VAE
    # Load the model parameters
    #join os VAE directory with best_model.pth
    model_path          = os.path.join(VAE_model_directory, 'best_model.pth')
    model_settings_path = os.path.join(VAE_model_directory, 'best_model_settings.json')

    # Load the model
    VAE_model = load_model(model_path, model_settings_path, device)
    ...
    model = model_class(hidden_size, deepness, frame_size, N_filter_bank, sampling_rate, VAE_model, VAE_latent_dim, VAE_atoms_size, atoms_number).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(dataloader, desc=f"Epoch {epoch + 1}/{num_epochs}"):
            ...
            if regularization:
                ...
                # print("Features reconstructed:\n", features_reconstructed)
                loss_regularizer = torch.sqrt(torch.nn.functional.mse_loss(features_og, features_reconstructed)) * 10
                print("Loss regularizer:", loss_regularizer)

            print("Loss main:", loss_main)
            print("Loss regularizer:", loss_regularizer)
            # Final loss
            loss_total = loss_main + loss_regularizer
