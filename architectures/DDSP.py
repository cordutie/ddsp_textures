...
class DDSP_textenv_gru(nn.Module):
    def __init__(self, hidden_size, N_filter_bank, deepness, compression, frame_size, sampling_rate, seed):
        ...
        self.r_encoder = mlp(1, hidden_size, deepness)
    ...
    def forward(self, spectral_centroid, rate):
        latent_vector = self.encoder(spectral_centroid, rate)
    ...
    def synthesizer(self, spectral_centroid, rate, target_loudness):
        latent_vector = self.encoder(spectral_centroid, rate)
        real_param, imag_param = self.decoder(latent_vector)
